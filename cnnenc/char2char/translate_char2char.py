...
sys.path.insert(0, "/home/development/palak/curriculum/cnnenc/char2char") # change appropriately
...
if __name__ == "__main__":
    ...
    parser.add_argument('-model', type=str, default="models/bn_hi/bi-char2char.grads.npz") # absolute path to a model (.npz file)
    parser.add_argument('-translate', type=str, help="bn_hi / cs_en / fi_en / ru_en", default="bn_hi") # which language?
    parser.add_argument('-saveto', type=str) # absolute path where the translation should be saved
    parser.add_argument('-which', type=str, help="dev / test1 / test2", default="test1") # if you wish to translate any of development / test1 / test2 file from WMT15, simply specify which one here
    ...
    data_path = "/home/development/palak/curriculum/data/" # change appropriately

    if args.which not in "dev test1 test2".split():
        raise Exception('1')

    if args.translate not in ["bn_hi", "cs_en", "fi_en", "ru_en"]: # Palak
        raise Exception('1')
    ...
    if args.many:
        ...
        dictionary = "%s%s/train/processed/all_%s-%s.%s.tok.pkl" % (lang, en, lang, en, lang)
        dictionary_target = "%s%s/train/processed/all_%s-%s.%s.tok.pkl" % (lang, en, lang, en, en) # Palak
        source = wmts[args.translate][args.which][0][0]
    ...
    curr_time = time.strftime("%d.%m_%H:%M", time.gmtime())
    saveto = "translation/" + args.translate + "_" + curr_time + ".trns"
