...
def train(
      ...
      maxlen_trg=400,  # maximum length of the description
      maxlen_sample=400,
      optimizer='rmsprop',
      batch_size=64,
      valid_batch_size=64,
    ...
    for epoch in xrange(max_epochs):
        ...
        # NOTE : after prepare_data they get converted to numpy lists
            ...
            # verbose
            if (numpy.mod(eidx, dispFreq) == 0) and (cidx == 1):
                ...
                for jj in xrange(numpy.minimum(5, x.shape[1])):
                    ...
                    if source_word_level:
                        ...
                        for ii, vv in enumerate(x[:, jj]):
                            ...
                            if vv in worddicts_r[0]:
                                source_.append(worddicts_r[0][vv].encode('utf-8'))
                            else:
                                source_.append('UNK'.encode('utf-8'))
                        print "".join(source_)
                    print 'Truth ', jj, ' : ',
                    if target_word_level:
                        for vv in y[:, jj]:
                            ...
                            if vv in worddicts_r[1]:
                                if use_bpe:
                                    print (worddicts_r[1][vv]).replace('@@', '').encode('utf-8'),
                                else:
                                    print worddicts_r[1][vv].encode('utf-8'),
                            else:
                                print 'UNK',
                        print
                    else:
                        ...
                        for vv in y[:, jj]:
                            if vv == 0:
                                break
                            if vv in worddicts_r[1]:
                                truth_.append(worddicts_r[1][vv].encode('utf-8'))
                            else:
                                truth_.append('UNK'.encode('utf-8'))
                        print "".join(truth_)
                    print 'Sample ', jj, ': ',
                    if stochastic:
                        ss = sample
                    ...
                    if target_word_level:
                        ...
                        for vv in ss:
                            if vv == 0:
                                break
                            if vv in worddicts_r[1]:
                                sample_.append(worddicts_r[1][vv].encode('utf-8'))
                            else:
                                sample_.append('UNK'.encode('utf-8'))
                        print "".join(sample_)
                    print

            # validate model on validation set and early stop if necessary
            if numpy.mod(eidx, validFreq) == 0 and (cidx == 1) and eidx != 0:
                use_noise.set_value(0.)
            ...
            if numpy.mod(eidx, saveFreq) == 0 and (cidx == 1) and eidx != 0:
                print 'Saving...',
    ...
    opt_file_name = '%s%s%s.%d.npz' % (model_path, save_file_name, '.grads', eidx)
    numpy.savez(file_name, history_errs=history_errs, uidx=uidx, eidx=eidx, cidx=cidx, **params)
    numpy.savez(opt_file_name, **optparams)
    if best_p is not None and saveFreq != validFreq:
        best_file_name = '%s%s.%d.best.npz' % (model_path, save_file_name, eidx)
        best_opt_file_name = '%s%s%s.%d.best.npz' % (model_path, save_file_name, '.grads',eidx)
